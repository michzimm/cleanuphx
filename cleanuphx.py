...
def set_vmedia_boot_policy(ucs_handle, sp_template_boot_policy_object, org_name):
    ...
    if sp_template_boot_policy_name in ("HyperFlex", "HyperFlex-sd"):
        mo = LsbootUsbFlashStorageImage(parent_mo_or_dn="org-root/org-"+org_name+"/boot-policy-"+sp_template_boot_policy_name+"/storage/local-storage", order="3")
    elif sp_template_boot_policy_name in ("HyperFlex-m5", "HyperFlex-m2pch"):
    ...
    elif sp_template_boot_policy_name in ("HyperFlex-m2r1"):
        mo = LsbootEmbeddedLocalLunImage(parent_mo_or_dn="org-root/org-"+org_name+"/boot-policy-"+sp_template_boot_policy_name+"/storage/local-storage", order="3")
    ucs_handle.add_mo(mo, True)
...
def monitor_esxi_prompt(sp_name, sp_kvm_ip):
    ...
    kvm_session.expect(":~]")
    logger.info ("   <> Successfully connected to ESXi CLI prompt on service profile: "+sp_name)
...
def monitor_phy_server_assoc(ucs_handle, phy_server_dn):
    ...
    while True:
        ...
        if association == "none" and availability == "available":
            logger.info ("   <> Physical server: "+phy_server_dn+" successfully dissassociated.")
            return
        else:
            time.sleep(60)
            timepassed += 60
            if timepassed >= timeout:
                logger.info ("timed out waiting for dissassociation of physical server: "+phy_server_dn)
                sys.exit()
...
def test_intersight_handle(intersight_handle):
    ...
    if results.status_code == 200:
        ...
        logger.info (Fore.RED+"There was a problem connecting to Intersight. Check internet connectivity and the API key file and then try again."+Style.RESET_ALL)
        logger.info ("\n")
...
def send_intersight_api(method,resource_path,query_params,moid=None,body=None,intersight_handle=intersight_handle):
    ...
    try:
        ...
        logger.info (Fore.RED+"There was a problem sending api call to Intersight"+Style.RESET_ALL)
        logger.info (e)
        logger.info (results)
...
def monitor_cimc_esxi_prompt(cimc_ip):
    ...
    kvm_session.sendline("Cisco123")
    kvm_session.expect(":~]")
    logger.info ("   <> Successfully connected to ESXi CLI prompt on CIMC: "+cimc_ip)
...
logger = logging.getLogger('cleanuphx')
logger.setLevel(logging.INFO)
fh = logging.FileHandler('./logs/{:%b-%d-%Y_%H:%M}.log'.format(datetime.now()))
...
logger.info ("\n")
logger.info (Style.BRIGHT+Fore.CYAN+"Version: "+Style.RESET_ALL+str(version))

logger.info ("\n")
logger.info (Style.BRIGHT+"WARNING!!!"+Style.RESET_ALL)
logger.info ("The following script will completely erase a HyperFlex configuration including the data, which will not be recoverable afterwards.")
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.GREEN+"TASK 1: Get Environment Details"+Style.RESET_ALL)
logger.info ("\n")


while True:

    logger.info (Style.BRIGHT+Fore.WHITE+"Choose the number that best describes your HyperFlex cluster:"+Style.RESET_ALL)
    logger.info ("\n")
    logger.info ("     1. Standard HyperFlex connected to Intersight")
    logger.info ("     2. Standard HyperFlex not connected to Intersight")
    logger.info ("     3. HyperFlex Edge connected to Intersight")
    logger.info ("     4. HyperFlex Edge not connected Intersight")
    logger.info("\n")
    cluster_type = input(Style.BRIGHT+Fore.WHITE+"     Selection: "+Style.RESET_ALL)
    if cluster_type in ("1","2","3","4"):
        break
    else:
        logger.info ("   <> Not a valid entry, please retry...")
...
if cluster_type in ("1","3"):
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"Gathering Intersight API Details..."+Style.RESET_ALL)
    logger.info ("\n")
    ...
    while True:
        ...
        if path.exists("./keys/"+intersight_api_file):
            ...
            logger.info ("      "+u'\U0001F44D'+" Done.")
            logger.info ("\n")
            break
        else:
            logger.info ("   <> Unable to locate provided API key file. please retry...")
            logger.info ("\n")
    ...
    while True:
        ...
        if intersight_cluster_exists == True:
            logger.info ("   <> Successfully found HyperFlex cluster in Intersight.")
            logger.info ("      "+u'\U0001F44D'+" Done.")
            break
        else:
            logger.info ("   <> Unable to find specified HyperFlex cluster in Intersight. Please check Intersight or re-enter cluster name...")
    logger.info ("\n")
...
if cluster_type in ("1","2"):
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"Gathering UCS Details..."+Style.RESET_ALL)
    logger.info ("\n")
    ...
    while True:
        ...
        try:
            ...
            if ucs_handle:
                logger.info ("   <> Successfully connected to UCS Manager.")
                logger.info ("      "+u'\U0001F44D'+" Done.")
                break
        except:
            logger.info ("   <> Unable to connect to UCS Mananger with the provided details, please retry...")
    ...
    while True:
        ...
        if org_exists(ucs_handle, org_name):
            logger.info ("   <> Successfully found UCS Org.")
            logger.info ("      "+u'\U0001F44D'+" Done.")
            break
        else:
            logger.info ("   <> Provided UCS Org does not exist, please retry...")
    ...
    while True:
        ...
        if vmedia_policy_exists(ucs_handle, vmedia_policy_name):
            logger.info ("   <> Successfully found vMedia policy.")
            logger.info ("      "+u'\U0001F44D'+" Done.")
            break
        else:
            logger.info ("   <> Provided UCS vMedia policy does not exist, please retry...")
...
if cluster_type in ("3","4"):
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"Gathering CIMC Details..."+Style.RESET_ALL)
    logger.info ("\n")

    if cluster_type in ("3"):
        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Getting list of CIMC IP addresses from Intersight..."+Style.RESET_ALL)
        cimc_ip_list = get_device_ip_list_by_cluster_name(intersight_cluster_profile_name)
        for cimc_ip in cimc_ip_list:
            logger.info ("   <> Item: HyperFlex Edge Node, CIMC IP: "+cimc_ip)
        logger.info ("      "+u'\U0001F44D'+" Done.")
        logger.info ("\n")
        ...
        while True:
            ...
            try:
                ...
                logger.info ("    <> Provided CIMC IP: "+cimc_ip+" does not appear to be a valid IPv4 address, please retry...")
    ...
    while True:
        ...
        try:
            ...
            if cimc_connect:
                logger.info ("   <> Successfully connected to CIMC using provided credentials")
                cimc_disconnect(cimc_handle)
                logger.info ("      "+u'\U0001F44D'+" Done.")
                break
        except:
            logger.info ("   <> Unable to connect to CIMC with the provided credentials, please retry...")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"Gathering HyperFlex ESXi ISO vMedia Details..."+Style.RESET_ALL)
    logger.info ("\n")
    ...
    while True:
        ...
        if cimc_vmedia_type in ("nfs","NFS","cifs","CIFS","www","WWW"):
            ...
            logger.info ("   <> Share type entered not valid, please retry...")
    ...
    if cimc_vmedia_type == "cifs" or cimc_vmedia_type == "www":
        while True:
            ...
            if user_prompt in ("n","N","no","No","NO"):
                ...
                logger.info ("   <> Not a valid response, please retry...")
    logger.info ("\n")
...
logger.info (Style.BRIGHT+Fore.CYAN+"Gathering vCenter Details..."+Style.RESET_ALL)
logger.info ("\n")
...
while True:
    ...
    try:
        ...
        logger.info ("   <> Unable to connect to vCenter with the provided details, please retry...")
...
while True:
    ...
    if dc_exists(vcenter_handle, vcenter_dc):
        logger.info ("   <> Successfully found VMware Datacenter.")
        break
    else:
        logger.info ("   <> Provided VMware Datacenter does not exist, please retry...")
...
while True:
    ...
    if cluster_exists(vcenter_handle, vcenter_dc, vcenter_cluster):
        logger.info ("   <> Successfully found VMware Cluster.")
        break
    else:
        logger.info ("   <> Provided VMware Cluster does not exist, please retry...")

vcenter_disconnect(vcenter_handle)
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.GREEN+"TASK COMPLETED: Get Environment Details"+Style.RESET_ALL)
logger.info ("\n")
...
if cluster_type in ("1","2"):
    ...
    logger.info (Style.BRIGHT+Fore.GREEN+"TASK: Re-image HyperFlex Nodes"+Style.RESET_ALL)
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Connecting to UCS Manager..."+Style.RESET_ALL)
    ucs_handle = ucs_connect(ucsm_ip, ucsm_user, ucsm_pass)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Fetching service profiles in the provided ucs org..."+Style.RESET_ALL)
    sp_objects = get_sps_in_org(ucs_handle, org_name)
    for sp_object in sp_objects:
        logger.info ("   <> Item: Service Profile, Name: "+sp_object.name+", DN: "+sp_object.dn)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Fetching related service profile template..."+Style.RESET_ALL)
    ...
    logger.info ("   <> Item: Service Profile Template, Name: "+sp_template_object.name+", DN: "+sp_template_object.dn)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Fetching related service profile template boot policy information..."+Style.RESET_ALL)
    ...
    logger.info ("   <> Item: Service Profile Template Boot Policy, Name: "+sp_template_boot_policy_object.name+", DN: "+sp_template_boot_policy_object.dn)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Fetching current service profile template vmedia policy information..."+Style.RESET_ALL)
    ...
    if not sp_template_vmedia_policy_dn:
        logger.info ("   <> Item: Service Profile Template vMedia Policy, Name: <None>, DN: <None>")
    else:
        sp_template_vmedia_policy_object = get_ucs_object_by_dn(ucs_handle, sp_template_vmedia_policy_dn)
        logger.info ("   <> Item: Current Service Profile Template vMedia Policy, Name: "+sp_template_vmedia_policy_object.name+", DN: "+sp_template_vmedia_policy_object.dn)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Setting new service profile template vmedia policy..."+Style.RESET_ALL)
    sp_template_object = set_sp_template_vmedia_policy(ucs_handle, sp_template_object, vmedia_policy_name)
    sp_template_vmedia_policy_dn = get_sp_template_vmedia_policy_dn(ucs_handle, sp_template_object)
    sp_template_vmedia_policy_object = get_ucs_object_by_dn(ucs_handle, sp_template_vmedia_policy_dn)
    logger.info ("   <> Item: New Service Profile Template vMedia Policy, Name: "+sp_template_vmedia_policy_object.name+", DN: "+sp_template_vmedia_policy_object.dn)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Setting vmedia policy as first boot item in service profile template boot policy..."+Style.RESET_ALL)
    set_vmedia_boot_policy(ucs_handle, sp_template_boot_policy_object, org_name)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Rebooting service profiles in provided ucs org..."+Style.RESET_ALL)
    for sp_object in sp_objects:
        sp_power_action(ucs_handle, sp_object.dn, "hard-reset-immediate")
        logger.info ("   <> Rebooting service profile: "+sp_object.name)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Disconnecting from UCS Manager..."+Style.RESET_ALL)
    ucs_disconnect(ucs_handle)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Going to sleep while HyperFlex nodes are re-imaged, this can take ~25-30 minutes due to multiple required reboots during install..."+Style.RESET_ALL)
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Waking up..."+Style.RESET_ALL)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Getting service profile kvm ip addresses..."+Style.RESET_ALL)
    sp_objects = get_sps_in_org(ucs_handle, org_name)
    sp_kvm_ips = get_sp_kvm_ips(ucs_handle, sp_objects)
    for key, value in sp_kvm_ips.items():
        logger.info ("   <> Item: Service Profile, Name: "+key+", KVM IP: "+value)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Waiting for access to ESXi CLI prompt for service profiles, this can take another couple of minutes..."+Style.RESET_ALL)
    threads = []
    for key, value in sp_kvm_ips.items():
        logger.info ("   <> Waiting to connect to ESXi CLI prompt on service profile: "+key)
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Gracefully powering-off service profiles..."+Style.RESET_ALL)
    for sp_object in sp_objects:
        sp_power_action(ucs_handle, sp_object.dn, "soft-shut-down-only")
        logger.info ("   <> Powering-off service profile: "+sp_object.name)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.GREEN+"TASK COMPLETED: Re-image HyperFlex Nodes"+Style.RESET_ALL)
    logger.info ("\n")
...
##############################
    ...
    logger.info (Style.BRIGHT+Fore.GREEN+"TASK: Clean-up HyperFlex Config in UCS Manager"+Style.RESET_ALL)
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Getting list of physical rack servers supporting HyperFlex service profiles..."+Style.RESET_ALL)
    phy_server_dns = get_phys_server_dns(ucs_handle, sp_objects)
    for phy_server in phy_server_dns:
        logger.info ("   <> Item: Physical Server, DN: "+phy_server)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Deleting org \""+org_name+"\" in UCS Manager..."+Style.RESET_ALL)
    delete_org(ucs_handle, org_name)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Waiting for complete dissassociation of physical servers..."+Style.RESET_ALL)
    threads = []
    for phy_server in phy_server_dns:
    ...
    logger.info (Style.BRIGHT+Fore.GREEN+"TASK COMPLETED: Clean-up HyperFlex Config in UCS Manager"+Style.RESET_ALL)
    logger.info ("\n")
...
if cluster_type in ("3","4"):
    ...
    logger.info (Style.BRIGHT+Fore.GREEN+"TASK: Re-Image HyperFlex Edge Nodes"+Style.RESET_ALL)
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Connecting to CIMC interfaces of HyperFlex Edge nodes..."+Style.RESET_ALL)
    cimc_handle_list = []
    for cimc_ip in cimc_ip_list:
        cimc_handle = cimc_connect(cimc_ip, cimc_user, cimc_pass)
        cimc_handle_list.append(cimc_handle)
        logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", Connected: True")
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Powering-off HyperFlex Edge nodes..."+Style.RESET_ALL)
    ...
    for cimc_handle in cimc_handle_list:
        while True:
            ...
            if cimc_power_state == "off":
                logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", Power State: off")
                break
            else:
                time.sleep(5)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Creating vMedia Mount on HyperFlex Edge nodes..."+Style.RESET_ALL)
    for cimc_handle in cimc_handle_list:
        create_cimc_vmedia_mount(cimc_handle, cimc_vmedia_share, cimc_vmedia_filename, cimc_vmedia_type)
        cimc_ip = get_cimc_ip(cimc_handle)
        logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", vMedia Mount Created: True")
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")

    try:
        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Modifying Boot Policy on HyperFlex Edge nodes..."+Style.RESET_ALL)
        for cimc_handle in cimc_handle_list:
            set_cimc_boot_policy(cimc_handle)
            cimc_ip = get_cimc_ip(cimc_handle)
            logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", Boot Policy Modified: True")
        logger.info ("      "+u'\U0001F44D'+" Done.")
        logger.info ("\n")


        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Powering-on HyperFlex Edge nodes..."+Style.RESET_ALL)
        for cimc_handle in cimc_handle_list:
        ...
        for cimc_handle in cimc_handle_list:
            while True:
                ...
                if cimc_power_state == "on":
                    logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", Power State: on")
                    break
                else:
                    time.sleep(5)
        logger.info ("      "+u'\U0001F44D'+" Done.")
        ...
        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Disconnecting from CIMCs..."+Style.RESET_ALL)
        for cimc_handle in cimc_handle_list:
            cimc_disconnect(cimc_handle)
        logger.info ("      "+u'\U0001F44D'+" Done.")
        logger.info ("\n")
        ...
        cimc_handle_list = []
        for cimc_ip in cimc_ip_list:
            logger.info ("   <> Connected to CIMC IP: "+cimc_ip)
            cimc_handle = cimc_connect(cimc_ip, cimc_user, cimc_pass)
            cimc_handle_list.append(cimc_handle)
        logger.info ("      "+u'\U0001F44D'+" Done.")
        logger.info ("\n")
        ...
        threads = []
        for cimc_ip in cimc_ip_list:
            logger.info ("   <> Waiting to connect to ESXi CLI prompt on CIMC IP: "+cimc_ip)
        ...
        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Gracefully Shutting Down HyperFlex Edge nodes..."+Style.RESET_ALL)
        for cimc_handle in cimc_handle_list:
        ...
        logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Deleting vMedia Mount on HyperFlex Edge nodes..."+Style.RESET_ALL)
        for cimc_handle in cimc_handle_list:
            delete_cimc_vmedia_mount(cimc_handle)
            logger.info ("   <> Item: HyperFlex Edge Node CIMC: "+cimc_ip+", vMedia Mount Deleted: True")
        logger.info ("      "+u'\U0001F44D'+" Done.")
        logger.info ("\n")


        logger.info (Style.BRIGHT+Fore.GREEN+"TASK COMPLETED: Re-Image HyperFlex Edge Nodes"+Style.RESET_ALL)
        logger.info ("\n")
...
if cluster_type in ("1","3"):
    ...
    logger.info (Style.BRIGHT+Fore.GREEN+"TASK: Clean-up HyperFlex Config in Intersight"+Style.RESET_ALL)
    logger.info ("\n")


    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Unassigning Nodes from HyperFlex Cluster Profile..."+Style.RESET_ALL)
    ...
    logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Deleteing HyperFlex Cluster Device from Intersight Device List..."+Style.RESET_ALL)
    delete_intersight_hyperflex_cluster(intersight_cluster_profile_moid)
    logger.info ("      "+u'\U0001F44D'+" Done.")
    logger.info ("\n")

    logger.info (Style.BRIGHT+Fore.GREEN+"TASK COMPLETED: Clean-up HyperFlex Config in Intersight"+Style.RESET_ALL)
    logger.info ("\n")
...
logger.info (Style.BRIGHT+Fore.GREEN+"TASK: Clean-up HyperFlex Config in vCenter"+Style.RESET_ALL)
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Connecting to vCenter..."+Style.RESET_ALL)
vcenter_handle = vcenter_connect(vcenter_ip, vcenter_user, vcenter_pass)
logger.info ("      "+u'\U0001F44D'+" Done.")
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Deleting HyperFlex vCenter extensions..."+Style.RESET_ALL)
...
if len(hx_extensions) == 1 and hx_extensions[0] == "com.springpath.sysmgmt":
    logger.info ("   <> Only one HyperFlex cluster found, also deleting \"com.springpath.sysmgmt\" extension.")
    delete_vcenter_extension(vcenter_handle, "com.springpath.sysmgmt")
logger.info ("      "+u'\U0001F44D'+" Done.")
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.CYAN+"-->"+Fore.WHITE+" Deleting HyperFlex ESXi cluster in vCenter..."+Style.RESET_ALL)
cluster_object.Destroy()
logger.info ("      "+u'\U0001F44D'+" Done.")
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.GREEN+"TASK 4 COMPLETED: Clean-up HyperFlex Config in vCenter"+Style.RESET_ALL)
logger.info ("\n")


logger.info (Style.BRIGHT+Fore.GREEN+"HyperFlex Reset Completed!!!"+Style.RESET_ALL)
logger.info ("\n")
